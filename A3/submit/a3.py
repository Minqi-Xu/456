...
def install_udp_middlebox_flow( source_dpid
    ...
    normal_path = get_shortest_path_between(get_networkx_topology_graph(), source_dpid, destination_dpid)
    scr_to_mid_path = get_shortest_path_between(get_networkx_topology_graph(), source_dpid, middlebox_dpid)
    mid_to_dst_path = get_shortest_path_between(get_networkx_topology_graph(), middlebox_dpid, destination_dpid)
    i = 0
    output_port = 0
    while i < len(scr_to_mid_path):
        matching = build_match_for( get_host_mac_from_dpid(source_dpid), get_host_mac_from_dpid(destination_dpid), get_host_ip_from_dpid(source_dpid), get_host_ip_from_dpid(destination_dpid)
                       , source_port, destination_port, get_input_port(scr_to_mid_path, scr_to_mid_path[i]))
        if i != len(scr_to_mid_path) - 1:
            output_port = get_ports_connecting(scr_to_mid_path[i], scr_to_mid_path[i+1])[0]
        else:
            output_port = get_host_port(scr_to_mid_path[i])
        flowmod = build_openflow_flowmod(matching, output_port)
        install_flowmod_on_switch_with_dpid(flowmod, scr_to_mid_path[i])
        i = i + 1
    i = 0
    while i < len(mid_to_dst_path):
        matching = build_match_for( get_host_mac_from_dpid(middlebox_dpid), get_host_mac_from_dpid(destination_dpid),  get_host_ip_from_dpid(middlebox_dpid), get_host_ip_from_dpid(destination_dpid)
                       , get_ports_connecting(mid_to_dst_path[0], mid_to_dst_path[1])[0], destination_port, get_input_port(mid_to_dst_path, mid_to_dst_path[i]))
        if i != len(mid_to_dst_path) - 1:
            output_port = get_ports_connecting(mid_to_dst_path[i], mid_to_dst_path[i+1])[0]
        else:
            output_port = get_host_port(mid_to_dst_path[i])
        flowmod = build_openflow_flowmod(matching, output_port)
        install_flowmod_on_switch_with_dpid(flowmod, mid_to_dst_path[i])
        i = i + 1
