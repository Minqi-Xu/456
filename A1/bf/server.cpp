...
int main(int argc, char *argv[]) {
    try {
        ...
        if(listen(server_socket, 1) == -1) {    // initialize listening
        ...
        while(1) {  // continue try to accept client request code
            receiver = accept(server_socket, (struct sockaddr*)&remoteaddr, &remoteaddr_len);
            if(receiver == -1) {
            ...
            isValid = true;
            int client_req = convert_without_throw(buff);   // convert the string content into int
            ...
            if(isValid == false || client_req != req_code) {    // if the request code is not valid or not matched close the connection
                ...
                send(receiver, &r_port_str, r_port_str.length(), 0);
                close(receiver);
                break;
            }
        ...
        server_socket = socket(AF_INET, SOCK_DGRAM, 0); // set the connection type be UDP
        memset(&address, 0, sizeof(address));
        memset(&remoteaddr, 0, sizeof(remoteaddr));
        memset(buff, 0 ,sizeof(buff));
        address.sin_family = AF_INET;
        ...
        while(1) {
            ...
            string info = buff;
            if(info.compare("EXIT")) { // if "EXIT" receives, close the connection
                break;
            }
            ...
            sendto(server_socket, &info, info.length(), 0, (struct sockaddr*)&remoteaddr, len);
        }
    ...
    } catch(const string err_msg) { // exception catch, print the error message
        cerr << err_msg << endl;
        return 1;
    }
}
