...
void incre_N() {    // use to increase N and will never exceed 10
    if(N >= 10) {
        N = 10;
        return;
    }
...
void incre_32(uint32_t* i) {       // use to increase an int and will never exceed 32, when meet 32, change into 0
    *i = *i + 1;
    if(*i == 32) {
        *i = 0;
    }
...
int main(int argc, char *argv[]) {
    try{
        ...
        // stores the file data into a string array namely datas[]
        if(numPacket != 0) {
            if(size % MAX_DATA_SIZE == 0) {
                myfile.read(tempbuff, MAX_DATA_SIZE);
        ...
        // initialize some important status variable
        ...
        for(current_load = 0; current_load < numPacket && current_load < 32; current_load++) {
            // load the packets into packqueue[], also update corresponding absi[]
            Packet temp;
            temp.type = 1;
        ...
        // TT is the timer that will store target, time, and whether it is running
        timer TT;
        TT.isRun = false;
        ...
        while(1) {
            ...
            if(!isSentEOT && next_send >= numPacket && sendbase >= next_send) {
                // if we send all packets, send an EOT to the receiver
                undefPacket eot;
                eot.length = htonl(0);
                eot.type = htonl(2);
                isSentEOT = true;
            ...
            if(buff.type == 0) {
                ...
                if(lastACK3 == buff.segnum && lastACK2 == buff.segnum && lastACK == buff.segnum && buff.segnum == TT.seqnum % 32) {
                    ...
                    if(absi[buff.segnum] <= lim && absi[buff.segnum] != -1) {
                        ...
                        if(absi[buff.segnum] != sendbase - 1) {
                            ...
                            if(next_send - sendbase <= 0) {
                                ...
                                for(; ptr != endat; incre_32(&ptr)) {
                                        // try to update the the packqueue (load more if possible)
                                    if(current_load < numPacket) {
                                        ...
                                        // else mark the packet in packqueue is invalid
                                        absi[ptr] = -1;
                                    }
                                    sendbase++;
                                }
                                next_send = max(next_send, sendbase);
                                // set the timer on another oldest unacked packet
                                TT.isRun = true;
                                if(next_send - sendbase <= 0) {
                                    TT.isRun = false;
                                }
                                TT.seqnum = sendbase % 32;
