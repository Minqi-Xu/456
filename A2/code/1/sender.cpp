        ...
        while(1) {
            ...
            if(next_send < numPacket) {
                if(next_send - sendbase < N) {
                    cerr << sizeof(packqueue[next_send%32]) << endl;
                    // window is not full
                    struct mystruct{
                        uint32_t type;
                        uint32_t segnum;
                        uint32_t length;
                        char data[packqueue[next_send%32].length];
                    };
                    mystruct sendPack;
                    ...
                    for(int i = 0; i < sendPack.legnth; i++) {
                        sendPack.data[i] = packqueue[next_send%32].data[i];
                    }
                    sendto(sending_socket, &sendPack, sizeof(sendPack), 0, (struct sockaddr*)&address2, len);
...
*/
            ...
            if(next_send >= numPacket && sendbase >= next_send) {
                /*
                Packet eot;
                eot.length = 0;
                eot.type = 2;
            ...
            if(buff.type == 0) {
                ...
                if(lastACK3 == buff.segnum && lastACK2 == buff.segnum && lastACK == buff.segnum && buff.segnum == TT.seqnum % 32) {
                    ...
                    // test whether the current ACK is transformed before
